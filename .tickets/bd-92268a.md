...
- [ ] Install dependencies:
  ```bash
  npm install -D oxlint eslint eslint-plugin-svelte svelte-eslint-parser @typescript-eslint/parser prettier prettier-plugin-svelte
  ```
- [ ] Create `oxlint.json` with ignorePatterns for `.svelte` files
- [ ] Create `eslint.config.js` for Svelte-only linting (flat config)
- [ ] Create `prettier.config.js` with svelte plugin
...
- [ ] Update `.husky/hooks/pre-commit` to run both linters on staged files
